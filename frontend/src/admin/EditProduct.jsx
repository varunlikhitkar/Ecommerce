...
const EditProduct = () => {
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '40px auto', background: '#FFFFFF', padding: '40px', borderRadius: '12px', border: '1px solid rgba(217, 92, 71, 0.22)' }}>
      <h2 style={{ color: '#D95C47', marginBottom: '20px' }}>Edit Product</h2>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        <div style={{ padding: '15px', border: '1px dashed #D95C47', borderRadius: '8px' }}>
          <label style={{ display: 'block', marginBottom: '10px', color: '#6B6763' }}>Replace Image (Optional)</label>
          <input type="file" accept="image/*" onChange={(e) => setImage(e.target.files[0])} style={{ color: '#2B2B2B' }} />
        </div>
...
const inputStyle = { padding: '12px', background: '#FFFFFF', border: '1px solid rgba(217, 92, 71, 0.25)', borderRadius: '6px', color: '#2B2B2B', fontSize: '15px', outline: 'none' };
